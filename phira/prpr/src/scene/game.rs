...
pub struct GameScene {
    ...
    exercise_btns: (RectButton, RectButton, RectButton),
    exercise_edit_target: Option<u8>,
    exercise_edit_text: String,
    exercise_edit_cursor: usize,
...
impl GameScene {
    ...
    pub async fn new(
        ...
        Ok(Self {
            ...
            exercise_range,
            exercise_press: None,
            exercise_btns: (RectButton::new(), RectButton::new(), RectButton::new()),
            exercise_edit_target: None,
            exercise_edit_text: String::new(),
            exercise_edit_cursor: 0,
...
impl Scene for GameScene {
    ...
    fn update(&mut self, tm: &mut TimeManager) -> Result<()> {
        ...
        if let Some(target) = self.exercise_edit_target {
            while let Some(c) = get_char_pressed() {
                if (c.is_ascii_digit() || c == ':' || c == '.' || c == '-') && self.exercise_edit_text.len() < 12 {
                    let cur = self.exercise_edit_cursor.min(self.exercise_edit_text.len());
                    self.exercise_edit_text.insert(cur, c);
                    self.exercise_edit_cursor = cur + 1;
                }
            }
            if is_key_pressed(KeyCode::Left) {
                self.exercise_edit_cursor = self.exercise_edit_cursor.saturating_sub(1);
            }
            if is_key_pressed(KeyCode::Right) {
                self.exercise_edit_cursor = (self.exercise_edit_cursor + 1).min(self.exercise_edit_text.len());
            }
            if is_key_pressed(KeyCode::Home) {
                self.exercise_edit_cursor = 0;
            }
            if is_key_pressed(KeyCode::End) {
                self.exercise_edit_cursor = self.exercise_edit_text.len();
            }
            if is_key_pressed(KeyCode::Backspace) {
                let cur = self.exercise_edit_cursor.min(self.exercise_edit_text.len());
                if cur > 0 {
                    self.exercise_edit_text.remove(cur - 1);
                    self.exercise_edit_cursor = cur - 1;
                }
            }
            if is_key_pressed(KeyCode::Delete) {
                let cur = self.exercise_edit_cursor.min(self.exercise_edit_text.len());
                if cur < self.exercise_edit_text.len() {
                    self.exercise_edit_text.remove(cur);
                }
            }
            if is_key_pressed(KeyCode::Escape) {
                self.exercise_edit_target = None;
            }
            if is_key_pressed(KeyCode::Enter) || is_key_pressed(KeyCode::KpEnter) {
                let offset = self.offset().min(0.);
                let text = self.exercise_edit_text.clone();
                match target {
                    0 => {
                    ...
                    2 => {
                        if let Some(t) = parse_time(&text) {
                            if !(offset as f64..=self.res.track_length).contains(&t) {
                                show_message(tl!("ex-time-out-of-range")).error();
                            } else {
                                tm.seek_to(t);
                                if t < 0. {
    ...
    fn touch(&mut self, tm: &mut TimeManager, touch: &Touch) -> Result<bool> {
        if self.mode == GameMode::Exercise && tm.paused() {
            ...
            if touch.phase == TouchPhase::Started && self.exercise_edit_target.is_some() {
                self.exercise_edit_target = None;
            }
        }
        Ok(false)
